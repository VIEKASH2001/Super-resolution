...
class IGNN(nn.Module):
    def __init__(self):
        ...
        n_neighbors = cfg.NETWORK.N_NEIGHBOR
        ...
        # define body module
        m_body1 = [ResBlock(n_feats, kernel_size, res_scale=cfg.NETWORK.RES_SCALE) for _ in range(pre_blocks)]

        m_body2 = [
            ResBlock(n_feats, kernel_size, res_scale=cfg.NETWORK.RES_SCALE) for _ in range(n_resblocks - pre_blocks)
        ]
        ...
        fuse_up = [conv(n_feats * 2, n_feats, kernel_size), conv(n_feats, n_feats, kernel_size)]
        ...
        if cfg.CONST.SCALE == 4:
            ...
            m_tail = [conv(n_feats, 3, kernel_size, act=False)]  # act=False important for relu!!!

        self.body1 = nn.Sequential(*m_body1)
        self.gcn = GCNBlock(
            # n_feats * (cfg.NETWORK.TEMPORAL_WIDTH - int(not cfg.NETWORK.OWN_SON)),
            n_feats,
            scale,
            k=n_neighbors,
            patchsize=patch_size,
    ...
    def forward(self, x_son, x):
        _, idx_k, diff_patch = self.graph(x_son, x)
        idx_k = [each_idx_k.detach() for each_idx_k in idx_k]
        if cfg.NETWORK.WITH_DIFF:
            diff_patch = [each_diff_patch.detach() for each_diff_patch in diff_patch]

        x = [self.sub_mean(x_i) for x_i in x]
        x0 = [self.head(x_i) for x_i in x]
        x1 = [self.body1(x0_i) for x0_i in x0]
        x1_lr, x1_hr = self.gcn(x1, idx_k, diff_patch)

        x1 = self.fuse_b(torch.cat([x1[cfg.NETWORK.TEMPORAL_WIDTH // 2], x1_lr], dim=1))
        x2 = self.body2(x1) + x0[cfg.NETWORK.TEMPORAL_WIDTH // 2]
        x = self.upsample(x2)
