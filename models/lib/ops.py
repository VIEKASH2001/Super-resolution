...
def im2patch(x, patchsize, stride, padding=None, returnpadding=False):
    ...
    x2col = mytorch.ops.im2col(xpad, [patchsize] * 2, [stride] * 2, [0, 0])
    if returnpadding:
...
def patch2im(x_patch, patchsize, stride, padding):
    padtop, padbottom, padleft, padright = padding
    counts = mytorch.ops.col2im(torch.ones_like(x_patch), [patchsize] * 2, [stride] * 2, [0, 0])
    x = mytorch.ops.col2im(x_patch.contiguous(), [patchsize] * 2, [stride] * 2, [0, 0])
    x = x / counts
