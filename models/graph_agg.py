...
class GraphAggregation(nn.Module):
    ...
    def aggregation(self, yd, idx_k):
        ...
        # yd -> b m f n
        # idx-> b m f k
        yd = [yd_i.permute(0, 2, 1).contiguous() for yd_i in yd]
        yd_e = [yd_i.view(b, 1, f, n).expand(b, m, f, n) for yd_i in yd]
        idx_k_e = [idx_k_i.view(b, m, 1, k).expand(b, m, f, k) for idx_k_i in idx_k]
        z = torch.cat([torch.gather(yd_e_i, dim=3, index=idx_k_e_i) for yd_e_i, idx_k_e_i in zip(yd_e, idx_k_e)], dim=3)

        # b m1*m2 c*p1*p2 k*(TEMPORAL_WIDTH - int(not OWN_SON))
        return z
    ...
    def forward(self, y, yd, idx_k):
        ...
        if cfg.NETWORK.OWN_SON:
            ...
            # Neglect own son
            sons = yd[: cfg.NETWORK.TEMPORAL_WIDTH // 2] + yd[1 + cfg.NETWORK.TEMPORAL_WIDTH // 2 :]

        y_patch, padding = zip(
            *[ops.im2patch(y_i, self.patchsize, self.stride, self.padding, returnpadding=True) for y_i in y]
        )
        yd_patch = [ops.im2patch(son_i, self.patchsize, self.stride, self.padding) for son_i in sons]

        # batch, channel, patchsize1, patchsize2, h, w
        _, _, H, W = y[cfg.NETWORK.TEMPORAL_WIDTH // 2].shape
        _, _, _, _, m1, m2 = y_patch[cfg.NETWORK.TEMPORAL_WIDTH // 2].shape
        b, c, p1, p2, n1, n2 = yd_patch[cfg.NETWORK.TEMPORAL_WIDTH // 2].shape

        m = m1 * m2
        n = n1 * n2
        f = c * p1 * p2
        k = self.k * (cfg.NETWORK.TEMPORAL_WIDTH - int(not cfg.NETWORK.OWN_SON))

        y_patch = [
            y_patch_i.permute(0, 4, 5, 1, 2, 3).contiguous().view(b, m, f // self.scale ** 2) for y_patch_i in y_patch
        ]
        yd_patch = [yd_patch_i.permute(0, 4, 5, 1, 2, 3).contiguous().view(b, n, f) for yd_patch_i in yd_patch]
        ...
        if cfg.NETWORK.WITH_ADAIN_NORM:
            reduce_scale = self.scale ** 2
            y_patch_norm = y_patch[cfg.NETWORK.TEMPORAL_WIDTH // 2].view(b, m, c // reduce_scale, p1 * p2)
            z_patch_norm = z_patch.view(b, m, c // reduce_scale, reduce_scale * p1 * p2, -1)
        ...
        z_patch_sr = (
            z_patch.view(b, k, c // self.scale ** 2, self.scale, self.scale, p1, p2, m1, m2)
            .permute(0, 1, 2, 5, 3, 6, 4, 7, 8)
            .contiguous()
        )
        z_patch_sr = z_patch_sr.view(b, k * (c // self.scale ** 2), p1 * self.scale, p2 * self.scale, m1, m2)
        padding_sr = [p * self.scale for p in padding[0]]
        # z_sr -> b k*c_y H*s W*s
