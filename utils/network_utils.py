...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 255)):
    ...
    if tensor.dim() == 4:
        ...
        img_np = tensor.numpy()
        img_np = np.transpose(img_np, (0, 2, 3, 1))[:, :, :, [2, 1, 0]]  # HWC, to BGR for cv2 imwrite
    ...
    if tensor.dim() == 3:
        ...
        raise Exception(f"Tensor dim must be either 3 or 4. Got dim {tensor.dim()}")
...
class Chop(nn.Module):
    ...
    def forward(self, x_son, x, shave=12, min_size=20000):
        n_GPUs = min(cfg.CONST.NUM_GPU, 4)
        b, c, h_s, w_s = x_son[0].size()
        _, _, h, w = x[0].size()
        ...
        lr_list_s = [
            [
                x_son_i[:, :, 0:h_size_s, 0:w_size_s],
                x_son_i[:, :, 0:h_size_s, (w_s - w_size_s) : w_s],
                x_son_i[:, :, (h_s - h_size_s) : h_s, 0:w_size_s],
                x_son_i[:, :, (h_s - h_size_s) : h_s, (w_s - w_size_s) : w_s],
            ]
            for x_son_i in x_son
        ]

        lr_list = [
            [
                x_i[:, :, 0 : h_size + h_0, 0 : w_size + w_0],
                x_i[:, :, 0 : h_size + h_0, (w - w_size - w_0) : w],
                x_i[:, :, (h - h_size - h_0) : h, 0 : w_size + w_0],
                x_i[:, :, (h - h_size - h_0) : h, (w - w_size - w_0) : w],
            ]
            for x_i in x
        ]
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 4, n_GPUs):
                lr_s_batch = [torch.cat(lr_list_s_i[i : (i + n_GPUs)], dim=0) for lr_list_s_i in lr_list_s]
                lr_batch = [torch.cat(lr_list_i[i : (i + n_GPUs)], dim=0) for lr_list_i in lr_list]
                sr_batch = self.model(lr_s_batch, lr_batch)
                sr_list.extend(sr_batch.chunk(n_GPUs, dim=0))
        else:
            sr_list = [
                self.forward(patch_s, patch, shave=shave, min_size=min_size)
                for (patch_s, patch) in zip(zip(*lr_list_s), zip(*lr_list))
            ]
        ...
        output = x[0].new(b, c, h, w)
        output[:, :, 0:h_half, 0:w_half] = sr_list[0][:, :, 0:h_half, 0:w_half]
        output[:, :, 0:h_half, w_half:w] = sr_list[1][:, :, 0:h_half, (w_size - w + w_half + w_0) : w_size + w_0]
        output[:, :, h_half:h, 0:w_half] = sr_list[2][:, :, (h_size - h + h_half + h_0) : h_size + h_0, 0:w_half]
        output[:, :, h_half:h, w_half:w] = sr_list[3][
            :, :, (h_size - h + h_half + h_0) : h_size + h_0, (w_size - w + w_half + w_0) : w_size + w_0
        ]
