...
def test(cfg, test_data_loader, net, Best_Epoch):
    ...
    for batch_idx, (img_name, img_lr_s, img_lr, img_hr) in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            img_out_y = net_utils.bgr2ycbcr(img_out_ / 255.0, only_y=True)
            img_hr_y = net_utils.bgr2ycbcr(img_hr_ / 255.0, only_y=True)
            ...
            if cfg.TEST.SAVEIMG:
                ...
                print(
                    "[TEST] [{0}/{1}]\t BT {2}\t PSNR {3}\t SSIM {4}\t Saving: {5}".format(
                        batch_idx + 1, n_batches, test_time, PSNRs, SSIMs, img_save_path
                    )
                )
            else:
                print(
                    "[TEST] [{0}/{1}]\t BT {2}\t PSNR {3}\t SSIM {4}".format(
                        batch_idx + 1, n_batches, test_time, PSNRs, SSIMs
                    )
                )
    ...
    print(
        "[TEST] [Best Epoch {0}] BatchTime_avg {1} PSNR_avg {2} SSIM_avg {3}\n".format(
            Best_Epoch, test_time.avg, PSNRs.avg, SSIMs.avg
        )
    )
    ...
    for i, name in enumerate(test_results["name"]):
        print(
            "[TEST]\t PSNR: %.3f \t SSIM: %.3f \t Name: %s" % (test_results["psnr"][i], test_results["ssim"][i], name)
        )
    result_file.close()
